...
const UiChart = ({
    ...
    const [config, setConfig] = useState<ReactApexChartProps.Props | undefined>(
        undefined
    );
    ...
    useEffect(() => {
        if (!value) return;
        const _value: number = Math.round(value[0] as number);
        ...
        const config: ChartConfig = {
            ...
            label: label,
            formatter: (val) =>
                props.formatter ? props.formatter(val) : val.toString(),
            maxValue: maxValue,
    ...
    const inValidInput = (...params: Limit[]) => {
        return params.some(
            (x) =>
                [x.low, x.high].some((x1) => +x1 < 0 || isNaN(x1)) ||
                +x.high < +x.low
        );
    };

    const onSaveLimit = () => {
        if ([iHigh, iLow].some((x) => !x || !x.current)) return;
        ...
        if (inValidInput(data)) {
            presentToast(
                "Input data invalid, please check again!",
                "danger",
                "top"
            );
            ...
            <IonCard className={`min-w-[150px] ${className}`}>
                <IonCardHeader>
                    ...
                    >
                        <IonChip color={"dark"} slot={"start"}>
                            {icon && (
                                <IonIcon icon={icon} color="primary"></IonIcon>
                            )}
                            <IonLabel
                                className={
                                    "text-grey-700 text-lg text-grey-700"
                                }
                            >
                                {title}
                            </IonLabel>
                ...
                <IonCardContent className={"h-[350px] md:h-[300px]"}>
                        ...
                        <IonGrid>
                            <IonRow>
                                <IonCol
                                    ...
                                    sizeMd={"4"}
                                    className={
                                        "flex justify-start items-center gap-x-0.5"
                                    }
                                >
                                    <IonText
                                        className={
                                            "text-md font-semibold text-gray-600 mr-2"
                                        }
                                    >
                                        H
                                    </IonText>
                    ...
                    {loading ? (
                        <div
                            className={
                                "w-full h-[220px] flex items-center justify-center"
                            }
                        >
                            <IonSkeletonText
                                animated={true}
